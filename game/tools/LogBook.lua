...
function LogBook.server_onCreate( self )
	...
	local locations = sm.storage.load( STORAGE_CHANNEL_LOCATIONS )

	local clientData = self:sv_getSaved()
	clientData.locations = locations
...
function LogBook.client_onCreate( self )
	...
	if self.tool:isLocal() then
		...
		self.cl.waypointGui = sm.gui.createWaypointIconGui()
...
function LogBook.client_onClientDataUpdate( self, data )
	if self.tool:isLocal() then
...
function LogBook.client_canEquip( _ )
	return Client_isQuestCompleted( quest_pickup_logbook )
end
...
function LogBook.cl_updateBeaconGui( self )
	...
	for _, beacon in pairs( beacons ) do
		local distance = math.floor( (characterPos - beacon.position):length() )
...
function LogBook.cl_updateLogGui( self, forceUpdate )
	if self.cl.updateLogGui or forceUpdate then
		...
		for _, uuid in ipairs( self.cl.logs ) do
			...
			table.insert( uiData, { uuid = uuid, read = read } )
		end
...
function LogBook.cl_addLog( self, uuid, notify )
	if uuid then
