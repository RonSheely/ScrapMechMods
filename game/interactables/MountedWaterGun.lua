...
function MountedWaterGun.sv_tryFire( self )
	...
	if logicInteractable and waterInteractable then
		local waterContainer = waterInteractable:getContainer( 0 )
		if logicInteractable:isActive() and not self.sv.parentActive and self.sv.canFire and waterContainer then
